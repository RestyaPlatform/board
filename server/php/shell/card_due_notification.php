...
if ($db_lnk) {
    ...
    while ($card = pg_fetch_assoc($result)) {
        ...
        foreach ($cards_users as $cards_user) {
            ...
            $message = 'Gentle reminder the card due date is tomorrow (<a href="' . $_server_domain_url . '/#/board/' . $card['board_id'] . '/card/' . $card['id'] . '">' . $card['name'] . '</a> on <a href="' . $_server_domain_url . '/#/board/' . $card['board_id'] . '">' . $card['board_name'] . '</a>)';
    ...
    if (!empty($users)) {
        foreach ($users as $key => $val) {
            $emailFindReplace['##CONTENT##'] = $val;
            sendMail('due_date_notification', $emailFindReplace, $key);
        }
    }
}
