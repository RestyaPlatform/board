    ...
    case '/settings': //settings update
        foreach ($r_post as $key => $value) {
            if (($key == 'LDAP_BIND_PASSWD' || $key == 'IMAP_EMAIL_PASSWORD')) {
                if (!empty($value)) {
                    $value = base64_encode(str_rot13($value));
                } else {
                    break;
                }
            }
