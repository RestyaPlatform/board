        ...
        */
        ...
        var core = {
            ...
            leftPanel: function (element) {
                ...
                $.each(element.data, function (i, entry) {
                    if (i >= element.pageNum * settings.itemsPerPage && i < (element.pageNum * settings.itemsPerPage + settings.itemsPerPage)) {
                        ...
                        entries.push('<span title="' + (entry.name || '') + '" class="fn-label' + (entry.cssClass ? ' ' + entry.cssClass : '') + '">' + (entry.name || '') + '</span>');
                        entries.push('</div>');

                        if (entry.desc) {
                            entries.push('<div class="row desc row' + i + ' " id="RowdId_' + i + '" data-id="' + entry.id + '">');
                            entries.push('<span title="' + (entry.desc || '') + '" class="fn-label' + (entry.cssClass ? ' ' + entry.cssClass : '') + '">' + entry.desc + '</span>');
                            entries.push('</div>');
                        }
            ...
            createProgressBar: function (days, cls, desc, label, dataObj) {
                ...
                var bar = $('<div class="bar"><div title="' + label + '" class="fn-label">' + label + '</div></div>')
                        .addClass(cls)
                        .css({
            ...
            synchronizeScroller: function (element) {
                if (settings.navigate === "scroll") {
                    ...
                    pos = pos > 0 ? 0 : (pos * -1 >= bWidth - (wButton * 0.75)) ? (bWidth - (wButton * 0.25)) * -1 : pos;
